...
require ('./conexion_db_cecati/conexion_cecati.php');
	...
	header("Content-Disposition: attachment;filename=Reporte_alumnos_becados.xls");
$cv_curso1 = ($_REQUEST['cv']);
$consulta_nc = "SELECT * FROM inscripciones2 ";
        $ejecutar_cons = $mysqli->query($consulta_nc);
$r_cons = $ejecutar_cons->fetch_assoc();
$row_cnt = $ejecutar_cons->num_rows;
...
<body>
	<div class="container">	
	<h3 class='center'>REPORTE DE ALUMNOS INSCRITOS: <?php echo $row_cnt ;?></h3>
		...
		<?php 			
			error_reporting(0);
			session_start();
				$sql = "SELECT * FROM inscripciones2 where especialidad != '' and curso_h != '' ORDER BY especialidad ASC";
				...
				while($filas = $ejecutar->fetch_row()){
					echo '<tr>
							...
							<td>'.$filas[3].'</td>
						  	...
						  	<td>'.$filas[8].'</td>
                            <td><a href="http://'.$_SERVER["cecati72.com.mx/"].'cecati72.com.mx/busqueda_beca.php?su=1&busqueda='.$filas[9].'">'.$filas[9].'</a></td>
                            <td>'.$filas[10].'</td>
                            <td>'.$filas[15].'</td>
                            <td>'.$filas[16].'</td>
                            <td>'.$filas[18].'</td>
