...
///////////////////consulta grafica///////////////////////
///////////////////administracion/////////////////////////
$consulta_admin = "SELECT * FROM inscripciones WHERE especialidad = 'ADMINISTRACIÓN' ";
$ejecuta_admin = $mysqli->query($consulta_admin);
$r_cons_admin = $ejecuta_admin->num_rows;
/////////////////////Asistencia ejecutiva////////////////////
$consulta_asis = "SELECT * FROM inscripciones WHERE especialidad = 'ASISTENCIA EJECUTIVA' ";
$ejecuta_asis = $mysqli->query($consulta_asis);
$r_cons_asis = $ejecuta_asis->num_rows;
///////////////////////Aplicacion de normas///////////////////
$consulta_apli = "SELECT * FROM inscripciones WHERE especialidad = 'APLICACIÓN DE NORMAS Y PROCEDIMIENTOS CONTABLES' ";
$ejecuta_apli = $mysqli->query($consulta_apli);
$r_cons_apli = $ejecuta_apli->num_rows;
/////////////////////////Elaboracion de dibujos////////////////
$consulta_dibujo = "SELECT * FROM inscripciones WHERE especialidad = 'ELABORACIÓN DE DIBUJOS ARQUITECTÓNICOS E INDUSTRIAL' ";
$ejecuta_dibujo = $mysqli->query($consulta_dibujo);
$r_cons_dibujo = $ejecuta_dibujo->num_rows;
///////////////////////soporte a instalaciones electricas////////////
$consulta_soporte_e = "SELECT * FROM inscripciones WHERE especialidad = 'SOPORTE A INSTALACIÓNES ELÉCTRICAS Y MOTORES ELÉCTRICOS' ";
$ejecuta_soporte_e = $mysqli->query($consulta_soporte_e);
$r_cons_soporte_e = $ejecuta_soporte_e->num_rows;
/////////////////////////Mantenimiento a equipos y sistemas electronicos/////////////////////
$consulta_mantenimiento_e = "SELECT * FROM inscripciones WHERE especialidad = 'MANTENIMIENTO A EQUIPOS Y SISTEMAS ELECTRÓNICOS' ";
$ejecuta_mantenimiento_e = $mysqli->query($consulta_mantenimiento_e);
$r_cons_mantenimiento_e = $ejecuta_mantenimiento_e->num_rows;
//////////////////////////Ofimatica////////////////////////////////
$consulta_ofi = "SELECT * FROM inscripciones WHERE especialidad = 'OFIMÁTICA' ";
$ejecuta_ofi = $mysqli->query($consulta_ofi);
$r_cons_ofi = $ejecuta_ofi->num_rows;
////////////////////////////USO DE LA LENGUA INGLESA EN DIVERSOS CONTEXTOS//////////////////////////
$consulta_ingles = "SELECT * FROM inscripciones WHERE especialidad = 'USO DE LA LENGUA INGLESA EN DIVERSOS CONTEXTOS' ";
$ejecuta_ingles = $mysqli->query($consulta_ingles);
$r_cons_ingles = $ejecuta_ingles->num_rows;
//////////////////////////////MANTENIMIENTO ELECTROMECÁNICO DEL AUTOMÓVIL///////////////////////////
$consulta_auto = "SELECT * FROM inscripciones WHERE especialidad = 'MANTENIMIENTO ELECTROMECÁNICO DEL AUTOMÓVIL' ";
$ejecuta_auto = $mysqli->query($consulta_auto);
$r_cons_auto = $ejecuta_auto->num_rows;
//////////////////////////////ELECTROMECÁNICA AUTOMOTRIZ/////////////////////////////
$consulta_electro = "SELECT * FROM inscripciones WHERE especialidad = 'ELECTROMECÁNICA AUTOMOTRIZ' ";
$ejecuta_electro = $mysqli->query($consulta_electro);
$r_cons_electro = $ejecuta_electro->num_rows;
//////////////////////////////REFRIGERACIÓN Y AIRE ACONDICIONADO////////////////////
$consulta_aire = "SELECT * FROM inscripciones WHERE especialidad = 'REFRIGERACIÓN Y AIRE ACONDICIONADO' ";
$ejecuta_aire = $mysqli->query($consulta_aire);
$r_cons_aire = $ejecuta_aire->num_rows;
      ...
      <div class="container" data-aos="fade-up">
        <div class="row">
          ...
          <div class="col-lg-5 pt-4 pt-lg-0 order-2 order-lg-1 content">
           <!-- <h3>//////////////////</h3><span class="icon-books"></span>-->
            <p class="font-italic">
<?php
$consulta_list_previa = "SELECT * FROM especialidades ORDER BY id ASC ";
           ...
           <!-- <th>Apellido materno &nbsp;</th>
            ...
            <tbody>
                ...
                while($filas = $ejecutar->fetch_row()){
                   ...
                   <!-- <td>'.$filas[0].'</td>-->
                    <td>'.$filas[1].'</td>
                  <!--  <td><a class="link_delete" href="reporte_word.php?id='.$filas[0] = hash('sha512',$filas[0]).'
&cv='.$filas[0].'"><span class="icon-microsoftexcel" ></span></a></td>-->
                    <td><a class="link_delete" href="editar_curso.php?id='.$filas[0] = hash('sha512',$filas[0]).'
&cv='.$filas[0].'"><span class="icon-pencil" style="color:green"></span></a></td>
                    <!--<td>$filas[4]</td>
                    <td>$filas[8]</td>-->
                    </tr>';
                } 
