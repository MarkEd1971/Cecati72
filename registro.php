...
if($id_c >= 1){
$consulta_nomc = "SELECT * FROM cursos WHERE id_c = $id_c ";
        $ejecutar_cons11 = $mysqli->query($consulta_nomc);
$r_cons11 = $ejecutar_cons11->fetch_assoc();
          $nom_c = $r_cons11['nombre_c'];
 }
...
//redireccionar  <META HTTP-EQUIV="Refresh" CONTENT="3; URL=log_in.php "></META>
 ...
 if($dia == '' or $mes == '' or $anio == '' or $sexo == '' or $edo_civil == '' or $curp == '' or $nacionalidad == '' or $nombre == '' or $apellido_p == '' or $apellido_m == '' or $correo == '' or $direccion == '' or $colonia == '' or $cp == '' or $localidad == '' or $municipio == '' or $estado == '' or $n_estudio == '' or $t_sangre == '' or $s_medico == '' or $enfermedad == '' or $discapacidad == '' or $motivo == '' or $medio == '' or $personas == '' or $estimacion ==''){
    ...
    $f_inscripcion3 ="INSERT INTO inscripciones (dia, mes, año, sexo, edo_civil ,curp, nacionalidad, nombre, nombre2, apellido_p, apellido_m, telefono, celular, correo, direccion, colonia, cp, localidad, municipio, estado, n_estudio, t_sangre, s_medico, enfermedad, discapacidad, motivo, medio, socio_e, personas, estimacion, n_empresa, p_trabajo, antiguedad, d_empresa, t_empresa, bienes,fecha ,hora) VALUES ('$dia','$mes','$anio','$sexo','$edo_civil','$curp','$nacionalidad','$nombre','$nombre2','$apellido_p','$apellido_m','$telefono','$celular','$correo','$direccion','$colonia','$cp','$localidad','$municipio','$estado','$n_estudio','$t_sangre','$s_medico','$enfermedad','$discapacidad','$motivo','$medio','$socio_e','$personas','$estimacion','$n_empresa','$p_trabajo','$antiguedad','$d_empresa','$t_empresa','$bienes_n','$fecha','$hora')";
                $f_inscripcion3 = $mysqli->query($f_inscripcion3); 
    
     if($cv_curso >= 1){
                     
                 $confirmacion .='<h2>Completaste el formulario,en breve será redireccionádo para la selección del horario del curso de '.$n_curso.'.</h2>'.'<br>';
 
                     echo '<META HTTP-EQUIV="Refresh" CONTENT="5; URL=seleccion_horario.php?id='.$uss = hash('sha512',$curp).'&token='.$curp.'&cv='.$cv_curso.'&id_c='.$id_c.'"></META>';
                }else{
                    $confirmacion .='<h2>Completaste el formulario,en breve será redireccionádo para la selección del curso.</h2>'.'<br>';
 
                     echo '<META HTTP-EQUIV="Refresh" CONTENT="5; URL=seleccion_horario.php?id='.$uss = hash('sha512',$curp).'&token='.$curp.'&id_c='.$id_c.'&cv='.$cv_curso.'"></META>';
                }
...
"></META>'; */
                ...
                if($cv_curso >= 1){
                 ...
                 $confirmacion .='<h2>Completaste el formulario,en breve será redireccionádo para la selección del horario del curso de '.$n_curso.'.</h2>'.'<br>';
                    ...
                    echo '<META HTTP-EQUIV="Refresh" CONTENT="5; URL=seleccion_horario.php?id='.$uss = hash('sha512',$curp).'&token='.$curp.'&id_c='.$id_c.'&cv='.$cv_curso.'"></META>';
                    /* echo '<META HTTP-EQUIV="Refresh" CONTENT="5; URL=seleccion_e.php?id='.$uss = hash('sha512',$curp).'&token='.$curp.'&cv='.$cv_curso.'&id_c='.$id_c.'"></META>';*/
                }else{
                    $confirmacion .='<h2>Completaste el formulario,en breve será redireccionádo para la selección del curso.</h2>'.'<br>';
 
                     echo '<META HTTP-EQUIV="Refresh" CONTENT="5; URL=seleccion_horario.php?id='.$uss = hash('sha512',$curp).'&token='.$curp.'&id_c='.$id_c.'&cv='.$cv_curso.'"></META>';
                }
               /*  echo '<META HTTP-EQUIV="Refresh" CONTENT="5; URL=envio_de_correo_curp.php?id='.$uss = hash('sha512',$uss).'&token='.$curp.'"></META>';*/
...
/////////////////////comprobacion de que la dimencion de la curp sea valida/////////////////
       ...
       /* $f_inscripcion3 ="INSERT INTO inscripciones (dia, mes, año, sexo, edo_civil ,curp, nacionalidad, nombre, nombre2, apellido_p, apellido_m, telefono, celular, correo, direccion, colonia, cp, localidad, municipio, estado, n_estudio, t_sangre, s_medico, enfermedad, discapacidad, motivo, medio, socio_e, personas, estimacion, n_empresa, p_trabajo, antiguedad, d_empresa, t_empresa, bienes,fecha ,hora,pago) VALUES ('$especialidad','$curso_h','$dia','$mes','$anio','$sexo','$edo_civil','$curp','$nacionalidad','$nombre','$nombre2','$apellido_p','$apellido_m','$telefono','$celular','$correo','$direccion','$colonia','$cp','$localidad','$municipio','$estado','$n_estudio','$t_sangre','$s_medico','$enfermedad','$discapacidad','$motivo','$medio','$socio_e','$personas','$estimacion','$n_empresa','$p_trabajo','$antiguedad','$d_empresa','$t_empresa','$bienes_n','$fecha','$hora','$pago')";
                $f_inscripcion3 = $mysqli->query($f_inscripcion3); 
...
<body>
  ...
  <main id="main">
    ...
    <!--  <div class="container">
        ...
        <h2>Bienvenido al registro</h2>
        <h3><?php if($id_c != ''){ echo 'Curso seleccionado previamente: '.$nom_c;} ?></h3>
    </div>
  </main>
  <footer id="footer">
  ...
  <div class="form-row">
    ...
    <div class="form-group col-md-4">
      ...
      <input type="email" class="form-control" name="correo" id="correo" placeholder="Correo" value="<?php echo $correo;?>" disabled>
    </div>
    ...
    <!--  <div class="form-group col-md-4">
      ...
      <label for="inputCity">Id de curso</label>
      <input type="text" class="form-control" name="id_c" id="id_c" placeholder="Escribe el estado" value="<?php echo $id_c;?>">
    </div>
      <div class="form-group col-md-2">
