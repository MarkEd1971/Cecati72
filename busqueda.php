...
if($elimina != ''){
    mysqli_query($mysqli,"Delete From inscripciones WHERE id = '$elimina'");
            echo '<META HTTP-EQUIV="Refresh" CONTENT="0.2; URL=busqueda.php?id='.$cv_curso1 = hash('sha512',$cv_curso1).'&cv='.$cv_curso1.'&busqueda='.$busqueda.' "></META>';

}
if($cambio >= 1){
    
     mysqli_query ($mysqli, "UPDATE inscripciones set pago = 'Pagado' WHERE id = '$cambio1'") or die ($errores .= '<li>Ha ocurrido un error en el servidor</li>');
         echo '<META HTTP-EQUIV="Refresh" CONTENT="0.2; URL=busqueda.php?id='.$cv_curso1 = hash('sha512',$cv_curso1).'&busqueda='.$busqueda.' "></META>';
...
<body>
  ...
  <br>
   ...
   // header('Location: perfil_su.php');
    ...
    $consulta_al = "SELECT * FROM inscripciones WHERE (curp LIKE '%$busqueda%' or telefono LIKE '%$busqueda%' or celular LIKE '%$busqueda%')";
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Servicio médico:<?php echo $s_medico;?></td>-->
        ...
        <br>
         <?php
  $consulta_list_previa = "SELECT * FROM inscripciones WHERE (curp LIKE '%$busqueda%' or telefono LIKE '%$busqueda%' or celular LIKE '%$busqueda%') ";
                ...
                while($filas = $ejecutar->fetch_row()){
                  //  $id= $filas[0];
                   ...
                   <!-- <td>'.$filas[0].'</td>-->
                    <td>'.$filas[2].'</td>
                    <td>'.$filas[3].'</td>
                    <td><a class="link_delete" href="busqueda.php?id='.$filas[1] = hash('sha512',$filas[1]).'
&cv='.$filas[0].'&c_v='.$filas[0].'&busqueda='.$busqueda.'"><span class="icon-bin" style="color:green"></span></a></td>
<td></td>
                    <td><a href="#">'.$filas[42].'</a></td>
                    <td><a href="busqueda.php?id='.$filas[1] = hash('sha512',$filas[1]).'
&cv='.$filas[0].'&busqueda='.$busqueda.'&cambio=1"><span class="icon-checkmark2" style="color:green"></a></td>
                   <!-- <td><a class="link_delete" href="inscripciones.php?id='.$filas[0] = hash('sha512',$filas[0]).'
